...
assign sdram_wr_ack = ((work_state == `W_TRCD) & ~sdram_rd_wr) | 
                      ( work_state == `W_WRITE)|
                      ((work_state == `W_WD) & (cnt_clk < sdram_wr_burst - 10'd2));

//读SDRAM响应信号
assign sdram_rd_ack = (work_state == `W_RD) & 
                      (cnt_clk >= 10'd1) & (cnt_clk < sdram_rd_burst + 10'd1);
...
//刷新计数器循环计数7812ns (64ms内完成全部8192=2^13行刷新操作)
always @ (posedge clk or negedge rst_n)
    if(!rst_n)
        cnt_refresh <= 11'd0;
    else if(cnt_refresh < CNT_REFRESH) // 64ms/8192 =7812ns
        cnt_refresh <= cnt_refresh + 11'd1;
    else
        cnt_refresh <= 11'd0;
...
always @ (posedge clk or negedge rst_n)
    if(!rst_n)
        sdram_ref_req <= 1'b0;
    else if(cnt_refresh == CNT_REFRESH - 11'd1) 
...
always @ (*) begin
    case (init_state)
        ...
        `I_DONE: begin  //初始化完成后,判断工作状态
            case (work_state)
                `W_IDLE  :cnt_rst_n <= 1'b0; //行有效：延时计数器启动
                `W_ACTIVE:cnt_rst_n <= 1'b1; //行有效延时计数结束后，清零计数器
                //潜伏期延时计数结束后，清零计数器
